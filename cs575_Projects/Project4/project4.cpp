...
void Disease ()
{
	while( NowYear <= 2019){
			...
			if( NowMonth == 5 || NowMonth == 6 || NowMonth == 7){
				tempScaleDisease += (int) NowNumDeer/5;
			}
			else if( NowNumDeer == 0){
				tempScaleDisease = 0;
			}
			else{
				tempScaleDisease -= (int) 5/NowNumDeer;
			}
...
void GrainDeer ()
{
	while( NowYear <= 2019){
		...
		tempNumDeer -= (int)(tempScaleDisease/2);
...
void Watcher ()
{
	while( NowYear <= 2019 ){
		...
		// DoneAssigning barrier:
		#pragma omp barrier
		printf("%d/%d\t%f\t%f\t%f\t%d\n", NowYear, NowMonth+1, (5./9.)*(NowTemp-32), NowPrecip, NowHeight*2.54, NowNumDeer);
		NowMonth ++;
		if (NowMonth == 12)
		{
			NowMonth = 0;
