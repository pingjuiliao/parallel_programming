...
int main( int argc, char *argv[ ] )
{
	...
	for( int t = 0; t < NUMTRIES; t++ )
	{
		...
		#pragma omp parallel for reduction(+:SumVolume)
		for( int i = 0; i < NUMNODES*NUMNODES; i++ )
		{
			...
			SumVolume += Height( iu, iv) * tileArea;
		}

		double time_next = omp_get_wtime( );
		double megaMults = (double)(NUMNODES*NUMNODES)/(time_next-time_now)/1000000.;
		time += (time_next-time_now);
	...
	cout << "Peak Performance == " << maxMegaMults << endl ;
	cout << "Volume == " << SumVolume/ (double) NUMTRIES << endl;
	return 0;
